...
def _geo_lookup(ip: str) -> dict:
    ...
    try:
        ...
        pass
    with GEO_LOCK:
        GEO_CACHE[ip] = {"country": country, "city": city, "ts": now}
        # 限制缓存大小
...
<body>
  ...
  <div class="card">
    ...
    <div class="row" style="margin-top:10px; flex-wrap: wrap; gap: 10px;">
      ...
      <span class="chip" id="visitorGeo">位置: --</span>
...
async function refreshOnline(){
  try{
    ...
    const geoEl = document.getElementById('visitorGeo');
    const onEl  = document.getElementById('onlineCount');
    if(ipEl) ipEl.textContent = 'IP: ' + (j.ip || '--');

    let loc = '--';
    const c1 = (j.country || '').trim();
    const c2 = (j.city || '').trim();
    if(c1 || c2){
      loc = c1 + (c1 && c2 ? ' · ' : '') + c2;
    }
    if(geoEl) geoEl.textContent = '位置: ' + loc;
    if(onEl) onEl.textContent = '在线: ' + (typeof j.online_count === 'number' ? j.online_count : '--');
  }catch(e){
    // ignore
  }
}
